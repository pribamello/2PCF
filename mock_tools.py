...
def make_randoms(bins, counts, Nr,  coord = 'xyz'):
    ...
    str       , coord   :    coordinate system. 'xyz' for cartesian 'ang' for angular on (1,1,1) cone basis. (STILL NEEDS IMPLEMENTATION)

    Returns: (xr, yr, zr) or (r, ra, dec)
    Random coordinate points for each particle, rotated so that they form a cone parallel to (1,1,1)

    WARNING: a cosmology is assumed in this function!!!
    '''
    # set cosmology
    h=0.6774
